...
public class AdminNewsIndexController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int numberOfItems = newsDAO.getItems().size();
		int numberOfPages = (int)(numberOfItems/DefineUtil.NUMBER_PER_PAGE);
		if(numberOfPages*DefineUtil.NUMBER_PER_PAGE<newsDAO.getItems().size()) {
			numberOfPages+=1;
		}
		int curentPage=1;
		try {
		...
		List<News> songListByPageAdmin = newsDAO.getItemsByPageAdmin(offset);
